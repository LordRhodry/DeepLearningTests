# -*- coding: utf-8 -*-
"""
Created on Mon May 29 14:31:23 2017
...
def generate_batch(batch_size, context_window):
    ...
    for i in range(batch_size):
        context = context_window  # target label at the center of the buffer
        contexts_to_avoid = [context_window]
        for j in range(context_size):  # can be improved as context tokens are fixed
            while context in contexts_to_avoid:
                context = random.randint(0, span - 1)
            contexts_to_avoid.append(context)
...
with graph.as_default():
    ...
    loss = tf.reduce_mean(
        tf.nn.nce_loss(weights=nce_weights, biases=nce_biases, inputs=embed_context, labels=train_labels,
